...
class Obsidian(Rock):
    ...
    def take_damage(self, damage, zombie):
        ...
        self.hp -= damage
        zombie.hp -= self.attack
        self.drawattack(self.screen, zombie)
        if self.hp <= 0:
            self.kill()
    ...
    def load_animation_frames(self): # アニメーションフレームを読み込む    
        for i in range(self.frames + 1):
            ...
            if os.path.exists(img_path):
                ...
                print(f"Warning: {img_path} not found")
        for i in range(self.attack_frames + 1):
            img_path = f"Assets/ObsidianAttack/ObsidianAttack_{i:05}.png"
            if os.path.exists(img_path):
                frame = pygame.image.load(img_path)
    ...
    def drawattack(self, screen, target):
        if self.attack_frame_count <= self.attack_frames:
            attack_frame = self.attack_animation_frames[self.attack_frame_count]  # 現在のフレームを選択
            screen.blit(attack_frame, (target.rect.x - 60, target.rect.y - 60))  # アニメーションフレームを描画
...
class Zombie(pygame.sprite.Sprite):
    ...
    def damage_to_plant(self, target):
        ...
        self.drawattack(self.instance.screen, target)
    
    def drawattack(self, screen, target):
        if self.frame_count <= 30:
            frame = self.animation_frames[self.frame_count]  # 現在のフレームを選択
            screen.blit(frame, (target.rect.x - 60, target.rect.y - 60))  # アニメーションフレームを描画
            self.frame_count += 1
        else:
            self.frame_count = 0
